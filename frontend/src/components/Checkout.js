...
async function placeOrderRequest(data, authToken, setProcessing, setCompleted) {
  console.log("start of function");

  const res = await fetch(`${SETTINGS.BASE_URL}/c/orders?action=create`, {
    method: "POST",
    headers: {
...
function PlaceOrder(props) {
  ...
  useEffect(() => {
    if (!processing && !completed) {
      ...
      placeOrderRequest(data, auth.user.token, setProcessing, setCompleted);
    }
  }, [auth, cart, props.orderDetails, processing, completed]);
